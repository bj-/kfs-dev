<?php $currentFileName = 'leaflet_popup_place_full_type1'; ?>
/*
//console.log('<?php var_dump($block["area_prefix"]) ?>');
*/
function <?= $currentFileName; ?>(data){

	//console.log(data);
	//console.log('--<?= $block["area_prefix"]; ?>');
	...
	if ( data && data?.interior?.[0]?.name )
	{
		data.interior.forEach((item, index) => {
			...
			//console.log(`${index}: ${aStart}${item.name}${aEnd}`);
		});
	}

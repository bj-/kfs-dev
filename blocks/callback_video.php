...
<style>
#<?= $bUID; ?> { 
	position: relative; background-size: cover !important; background-position: center !important; height: fit-content; margin-top: clamp(10px, 5vw, 50px); margin-bottom: clamp(10px, 2vw, 50px);
	& .fade { <?= $fade_style ?> width: 100%; height: 100%; }

	& .content { display: flex; gap: clamp(20px, 3vw, 50px); flex-wrap: wrap; align-items: center; }
	& .item { display: flex; justify-content: center; /* align-items: center; */ height: 100%; width: 100%; flex-direction: column; flex: 1 1 0; min-width: 150px; }

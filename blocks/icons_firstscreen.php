...
#<?= $bUID; ?> { 
	...
	& .title > h3 { font-size: clamp(28px, 3vw, 50px);  color: <?= $bender_settings["colors"]["link_second"]["color"] ?>; margin-top: 20px; margin-bottom: 20px; font-weight: normal;}
	& .content { display: flex; gap: 20px; padding-top: 50px; flex-wrap: wrap; }
	& .item { 
		display: flex; justify-content: center; align-items: center; height: 100%; flex-direction: column; flex: 1 1 0; 
		/* min-width: 240px; */ gap: clamp(10px, 2vw, 50px); 
	}
	& .item > img { max-width: 64px; filter: invert(1); }
	& .item-title { font-size: clamp(14px, 2.16vw, 20px); line-height: clamp(24px, 3vw, 35px); font-weight: 400; text-align: center;}
...
@media (max-width: 900px) {
	#<?= $bUID; ?> { 
		& .content { flex-direction: column; align-items: baseline; }
		& .item { flex-direction: row; gap: clamp(10px, 5vw, 45px); }
		& .item-title { text-align: left; font-size: clamp(14px, 3.5vw, 20px);}
	}
}

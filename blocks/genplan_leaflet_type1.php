...
//$genplan_areas = convertToLeafletArrayPlace($block["areas"], $block["coords"]["place"], $block["coords"]["shift"], $block["area_prefix"]);
...
//echo "<pre>block[area_prefix]\n"; var_dump($block["area_prefix"]); echo "</pre>";
...
/* Разрешаем попапам «выходить» за границы контейнера */
/*
#genplan .content,
#genplan_map {
...
(function() {
	...
	const topPadding = 200; // Пиксели «воздуха» сверху карты для попапов
...
<?php
	function processPopupsRecursive($array, $template_dir, $prefix = '', $bUID)
	{
		...
		//echo "<pre>blockXXXXXXXXXXXXXX\n"; var_dump($block); echo "</pre>";
		//echo "console.log('sfsdf" . $block . "');";
...
<?php endif; ?>
		...
		const popup = L.popup({
			...
			/*
			// ===== НОВЫЕ ПАРАМЕТРЫ ДЛЯ АВТО-ПАНОРАМИРОВАНИЯ =====
			autoPan: true,  // Включаем авто-сдвиг карты
			autoPanPadding: [50, 50],  // Отступы от краёв [horizontal, vertical]
			autoPanPaddingTopLeft: [null, topPadding + 40],  // Критично: отступ сверху под заголовок + паддинг
			autoPanPaddingBottomRight: [null, 30],  // Отступ снизу
			keepInView: true,  // Удерживать попап в видимой области
			// ================================================
			maxWidth: 350,
			className: 'genplan-popup'
			*/
